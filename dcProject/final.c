...
// Add these to the global definitions section
#define MAX_WARNING_LOGS 100

// Add this to the global variables section
typedef struct {
    char timestamp[20];
    char message[256];
    int severity; // 1 = warning, 2 = critical
} WarningLog;

WarningLog warning_logs[MAX_WARNING_LOGS];
int warning_log_count = 0;
pthread_mutex_t warning_log_mutex = PTHREAD_MUTEX_INITIALIZER;
...
void log_warning(int severity, const char *format, ...) {
    if (!warning_enabled) return;
    
    pthread_mutex_lock(&warning_log_mutex);
    
    // Only store if we have space
    if (warning_log_count < MAX_WARNING_LOGS) {
        WarningLog *log = &warning_logs[warning_log_count++];
        ...
        // Print current warning to console
        if (severity == 2) {
    ...
    pthread_mutex_unlock(&warning_log_mutex);
}

// Add this function to display warning logs
void display_warning_logs() {
    pthread_mutex_lock(&warning_log_mutex);
    
    if (warning_log_count == 0) {
        printf("No warnings have been logged.\n");
    } else {
        printf("\n=== Warning Log (%d entries) ===\n", warning_log_count);
        for (int i = 0; i < warning_log_count; i++) {
            if (warning_logs[i].severity == 2) {
                printf("[%s] \033[1;31m[CRITICAL]\033[0m %s\n", 
                       warning_logs[i].timestamp, warning_logs[i].message);
            } else {
                printf("[%s] \033[1;33m[WARNING]\033[0m %s\n", 
                       warning_logs[i].timestamp, warning_logs[i].message);
            }
        }
    }
...
// Now modify the check_network_health function to use the new log_warning function
void check_network_health() {
    pthread_mutex_lock(&metrics_mutex);
    double packet_loss = metrics.current_packet_loss;
    double jitter = metrics.current_jitter;
    ...
    if (packet_loss > CRITICAL_THRESHOLD_LOSS || 
        (traffic_type != TRAFFIC_UNKNOWN && packet_loss > acceptable_loss * 2)) {
        log_warning(2, "Severe packet loss detected: %.2f%% (Acceptable: %.2f%%)", 
               packet_loss, acceptable_loss);
    } 
    else if (packet_loss > WARNING_THRESHOLD_LOSS || 
             (traffic_type != TRAFFIC_UNKNOWN && packet_loss > acceptable_loss)) {
        log_warning(1, "Packet loss detected: %.2f%% (Acceptable: %.2f%%)", 
               packet_loss, acceptable_loss);
    }
    
    if (jitter > CRITICAL_THRESHOLD_JITTER || 
        (traffic_type != TRAFFIC_UNKNOWN && jitter > acceptable_jitter * 2)) {
        log_warning(2, "High jitter detected: %.2f ms (Acceptable: %.2f ms)", 
               jitter, acceptable_jitter);
    } 
    else if (jitter > WARNING_THRESHOLD_JITTER || 
             (traffic_type != TRAFFIC_UNKNOWN && jitter > acceptable_jitter)) {
        log_warning(1, "Elevated jitter detected: %.2f ms (Acceptable: %.2f ms)", 
               jitter, acceptable_jitter);
    }
    
    // Issue traffic-specific warnings - these don't need to be logged each time
    if (traffic_type != TRAFFIC_UNKNOWN) {
        printf("Detected primary traffic type: %s\n", traffic_profiles[traffic_type].name);
        
        // Specific recommendations based on traffic type
        if (traffic_type == TRAFFIC_GAMING && jitter > acceptable_jitter) {
            log_warning(1, "High jitter may cause lag in games");
        } 
        else if (traffic_type == TRAFFIC_VOIP && (packet_loss > acceptable_loss || jitter > acceptable_jitter)) {
            log_warning(1, "Network conditions may cause poor call quality");
        }
        else if (traffic_type == TRAFFIC_VIDEO_STREAMING && packet_loss > acceptable_loss) {
            log_warning(1, "Packet loss may cause video buffering or quality reduction");
        }
    }
}

// Finally, add the warnings command to the CLI handler
// In the cli_handler function, add this case in the command processing section
else if (strcmp(input, "warnings") == 0) {
    display_warning_logs();
}
void print_help_command() {
    printf("\nAvailable commands:\n");
    ...
    printf("  warnings  - Display all logged warnings\n");
