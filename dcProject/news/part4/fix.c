...
// Function to find available network interface
char* find_network_interface() {
    char errbuf[PCAP_ERRBUF_SIZE];
    pcap_if_t *alldevs, *dev;
    static char interface[256];
    ...
    if (alldevs == NULL) {
        fprintf(stderr, "No interfaces found! Make sure you have permissions.\n");
        return NULL;
    }
    
    // Use the first available interface
    dev = alldevs;
    strncpy(interface, dev->name, sizeof(interface)-1);
...
void start_server() {
    ...
    char *interface = find_network_interface();
    if (interface == NULL) {
        fprintf(stderr, "Could not find available interface for packet capture.\n");
        fprintf(stderr, "Continuing without packet capture functionality.\n");
    } else {
        ...
        if (handle == NULL) {
            ...
            printf("Packet capture started on interface: %s\n", interface);
        }
    }
    
    // Open pcap file for writing only if we have a valid handle
    pcap_dumper_t *dumper = NULL;
    if (handle != NULL) {
        dumper = pcap_dump_open(handle, "chat_packets.pcap");
        if (dumper == NULL) {
            fprintf(stderr, "Error opening pcap file for writing\n");
            pcap_close(handle);
            handle = NULL;
        }
    }
    ...
    while (1) {
        ...
        // Set timeout for select to allow packet processing
        ...
        // Capture packets only if we have a valid handle and dumper
